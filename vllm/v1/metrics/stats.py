...
class RequestStateStats:
    ...
    def get_timing_intervals(self) -> tuple[float, float, float, float]:
        if self.scheduled_ts == 0.0:
            return 0.0, 0.0, 0.0, 0.0

        queued_time = max(self.scheduled_ts - self.queued_ts, 0.0)

        if self.first_token_ts == 0.0:
            return queued_time, 0.0, 0.0, 0.0

        prefill_time = max(self.first_token_ts - self.scheduled_ts, 0.0)
        decode_time = max(self.last_token_ts - self.first_token_ts, 0.0)
        inference_time = max(self.last_token_ts - self.scheduled_ts, 0.0)
        return queued_time, prefill_time, decode_time, inference_time
...
class IterationStats:
    ...
    def update_from_output(
        ...
        has_started = req_stats.scheduled_ts != 0.0
        is_queued_abort = (
            is_prefilling
            and output.finish_reason == FinishReason.ABORT
            and not has_started
        )

        if is_prefilling and not is_queued_abort:
        ...
        if is_prefilling and not is_queued_abort:
            ...
            if req_stats.last_token_ts != 0.0:
                itl = engine_core_timestamp - req_stats.last_token_ts
                self.inter_token_latencies_iter.append(itl)

        if not is_queued_abort:
    ...
    def update_from_finished_request(
        ...
        queued_time, prefill_time, decode_time, inference_time = (
            req_stats.get_timing_intervals()
        )
